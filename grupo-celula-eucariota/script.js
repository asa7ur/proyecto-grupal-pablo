...
function cargarDatos() {
  ...
  tbody.innerHTML = '<tr><td colspan="2" style="text-align:center; padding:40px; font-size:18px;">🔄 Consultando base de datos...</td></tr>';
  ...
  fetch(API_URL + "?dia=" + dia)
    ...
    .then((data) => {
      ...
      let agenda = {};
      const initHora = (h) => { if (!agenda[h]) agenda[h] = { guardias: [], faltas: [] }; };
      ...
      let horasPresentes = Object.keys(agenda).sort((a, b) => ORDEN_VISUAL.indexOf(a) - ORDEN_VISUAL.indexOf(b));
      ...
      horasPresentes.forEach((hora) => {
        ...
        let htmlGuardias = info.guardias.length > 0 
...
function fetchEucariota() { cargarDatos(); }
...
async function fetchJotasones() {
  ...
  } catch (e) { alert("Error Jotasones (Puerto 3000)"); }
}

async function fetchMoteros() {
  const diaStr = document.getElementById("selDia").value;
  try {
    const res = await fetch(`http://localhost:3001/api/panel?diaSemana=${diaStr}`);
    const data = await res.json();
  ...
  } catch (e) { alert("Error Moteros (Puerto 3001)"); }
}
...
async function fetchDuostream() {
  ...
  } catch (e) { alert("Error al leer CSV de Duostream"); }
}